...
class TestSageAttentionWiring:
    ...
    def test_sage_attention_config_defaults(self):
        from squish.attention.sage_attention import SageAttentionConfig
    ...
    def test_sparge_config_defaults(self):
        from squish.attention.sparge_attn import SpargeAttnConfig
        cfg = SpargeAttnConfig()
        assert 0 < cfg.sparse_threshold < 1

    def test_squeeze_init(self):
        from squish.attention.squeeze_attention import LayerKVBudget, SqueezeConfig, SqueezeKVCache
    ...
    def test_squeeze_layer_budget_default_score(self):
        from squish.attention.squeeze_attention import LayerKVBudget
...
class TestYOCOWiring:
    ...
    def test_robust_scheduler_init(self):
        from squish.serving.robust_scheduler import AMaxScheduler, RobustSchedulerConfig
...
class TestRobustSchedulerOps:
    def test_amax_scheduler_enqueue(self):
        ...
        from squish.serving.robust_scheduler import (
            AMaxScheduler,
            LengthInterval,
            Request,
...
class TestSparseSpecWiring:
    ...
    def test_fr_spec_calibrator_exists(self):
        from squish.speculative.fr_spec import FRSpecCalibrator, FRSpecConfig
...
class TestAdaptiveLayerOps:
    ...
    def test_layer_skip_decoder(self):
        from squish.token.layer_skip import EarlyExitConfig, EarlyExitDecoder
    ...
    def test_ipw_tracker_init(self):
        from squish.token.ipw import IPWConfig, IPWTracker
        tracker = IPWTracker(IPWConfig())
        assert tracker is not None

    def test_ipw_measurement_record(self):
        from squish.token.ipw import IPWConfig, IPWMeasurement, IPWTracker
        tracker = IPWTracker(IPWConfig())
...
class TestAlreadyWiredTierB:
    ...
    def test_kv_share_map_init(self):
        from squish.kv.kvsharer import KVShareMap, KVSharerConfig
        cfg = KVSharerConfig()
    ...
    def test_paris_kv_codebook_init(self):
        from squish.kv.paris_kv import ParisKVCodebook, ParisKVConfig
    ...
    def test_spec_reason_orchestrator_init(self):
        from squish.speculative.spec_reason import SpecReasonConfig, SpecReasonOrchestrator
        def _draft(ctx: str):
            from squish.speculative.spec_reason import ReasoningStep
...
class TestIntegrationReportAllWired:
    ...
    def test_all_34_modules_wired(self):
        ...
        import re
        server_src = (Path(__file__).parent.parent.parent / "squish" / "server.py").read_text()
