...
Unit tests for Phase 2 memory-management modules:
  - squish.io.split_loader  (CPU/GPU split loading)
  - squish.attention.flash_attention (Flash Attention wrapper)
  - squish.hardware.layerwise_loader (AirLLM-style layer streaming)
...
class TestSplitLoaderHelpers:
    ...
    def test_layer_weight_bytes_counts_all_params(self):
        ...
        from squish.io.split_loader import _layer_weight_bytes
        ...
        with patch("squish.io.split_loader._mx") as mock_mx_fn:
...
class TestSplitLayerLoaderLogic:
    ...
    def test_auto_split_model_fits_returns_none(self):
        ...
        with patch("squish.io.split_loader._get_metal_limit_bytes", return_value=16 * 1024 ** 3):
            model = _make_mock_model(n_layers=8, mb_per_layer=10.0)  # total ~80 MB << 16 GB
            # Patch _layer_weight_bytes to return known values
            with patch("squish.io.split_loader._layer_weight_bytes", return_value=10 * 1024 ** 2):
    ...
    def test_apply_with_forced_cpu_layers(self):
        ...
        from squish.io.split_loader import OffloadedLayer, SplitLayerLoader

        model = _make_mock_model(n_layers=4, mb_per_layer=10.0)

        with patch("squish.io.split_loader._get_metal_limit_bytes", return_value=16 * 1024 ** 3):
            with patch("squish.io.split_loader._layer_weight_bytes", return_value=10 * 1024 ** 2):
                with patch("squish.io.split_loader._flatten_params", return_value=[("w", np.zeros((4,), dtype=np.float16))]):
    ...
    def test_profile_model_layers_returns_list(self):
        from squish.io.split_loader import profile_model_layers

        model = _make_mock_model(n_layers=4, mb_per_layer=50.0)
        with patch("squish.io.split_loader._layer_weight_bytes", return_value=50 * 1024 ** 2):
...
class TestFlashAttentionAvailability:
    ...
    def test_is_flash_attention_available_type(self):
        from squish.attention.flash_attention import _has_fast_sdp_available
        result = _has_fast_sdp_available()
        assert isinstance(result, bool)

    def test_patch_result_default_zero(self):
        from squish.attention.flash_attention import PatchResult
        r = PatchResult()
    ...
    def test_patch_model_no_layers(self):
        from squish.attention.flash_attention import PatchResult, patch_model_attention
        model = MagicMock()
    ...
    def test_attention_status_returns_dict(self):
        from squish.attention.flash_attention import attention_status
    ...
    def test_predict_memory_savings_shape(self):
        from squish.attention.flash_attention import predict_memory_savings
...
class TestFlashAttentionBenchmark:
    ...
    def test_benchmark_skipped_without_fast_sdp(self):
        pytest.importorskip("mlx.core")
        from squish.attention.flash_attention import _has_fast_sdp_available, benchmark_attention
...
class TestLayerCache:
    ...
    def test_lru_eviction(self):
        from squish.hardware.layerwise_loader import LayerCache
        ...
        with patch("squish.hardware.layerwise_loader._zero_layer_weights"):
...
class TestShardModel:
    ...
    def test_shard_model_creates_directories(self, tmp_path):
        from squish.hardware.layerwise_loader import _LAYER_META_FILE, _MODEL_META_FILE, shard_model
        ...
        with patch("squish.hardware.layerwise_loader._flatten_params", return_value=fake_params):
    ...
    def test_shard_model_meta_content(self, tmp_path):
        from squish.hardware.layerwise_loader import _FORMAT_VERSION, _MODEL_META_FILE, shard_model
    ...
    def test_shard_model_empty_raises(self, tmp_path):
        from squish.hardware.layerwise_loader import shard_model
...
class TestRecommendCacheSize:
    ...
    def test_basic_calculation(self):
        from squish.hardware.layerwise_loader import recommend_cache_size
...
class TestLoadStatsStr:
    ...
    def test_zero_stats(self):
        from squish.hardware.layerwise_loader import LoadStats
        s = LoadStats()
