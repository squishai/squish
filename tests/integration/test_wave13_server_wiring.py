...
class TestDuoAttentionWiring:
    def test_import(self):
        from squish.attention.duo_attention import DuoAttentionConfig, DuoKVManager
    ...
    def test_config_defaults(self):
        from squish.attention.duo_attention import DuoAttentionConfig
    ...
    def test_head_classifier_import(self):
        from squish.attention.duo_attention import DuoAttentionConfig, HeadClassifier
...
class TestShadowKVWiring:
    ...
    def test_config_defaults(self):
        from squish.kv.shadow_kv import ShadowKVConfig
        cfg = ShadowKVConfig()
...
class TestPQCacheWiring:
    def test_import(self):
        from squish.kv.pq_cache import PQCacheConfig, PQKeyIndex
        cfg = PQCacheConfig(n_subvectors=4, n_codes=8)
        idx = PQKeyIndex(dim=32, config=cfg)
        assert idx is not None

    def test_config_defaults(self):
        from squish.kv.pq_cache import PQCacheConfig
        cfg = PQCacheConfig()
    ...
    def test_add_and_retrieve(self):
        from squish.kv.pq_cache import PQCacheConfig, PQKeyIndex, PQValueStore, retrieve
...
class TestSpeCacheWiring:
    def test_import(self):
        from squish.kv.spe_cache import InMemoryBlockStore, SpeCacheConfig, SpeCachePrefetcher
    ...
    def test_config_defaults(self):
        from squish.kv.spe_cache import SpeCacheConfig
        cfg = SpeCacheConfig()
...
class TestDuoDecodingWiring:
    def test_import(self):
        from squish.attention.duo_decoding import (
            DuoCPUVerifier,
            DuoDecodingConfig,
            DuoDecodingDecoder,
            DuoScheduler,
    ...
    def test_config_defaults(self):
        from squish.attention.duo_decoding import DuoDecodingConfig
...
class TestKnapSpecWiring:
    def test_import(self):
        from squish.token.knapspec import KnapSpecConfig, KnapSpecSelector
        cfg = KnapSpecConfig(num_layers=4, budget_fraction=0.5)
        sel = KnapSpecSelector(cfg)
        assert sel is not None

    def test_config_defaults(self):
        from squish.token.knapspec import KnapSpecConfig
        cfg = KnapSpecConfig()
...
class TestTokenMergingWiring:
    ...
    def test_merge_reduces_length(self):
        from squish.token.token_merging import TokenMergingConfig, bipartite_merge
        rng    = np.random.default_rng(0)
    ...
    def test_unmerge_restores_length(self):
        from squish.token.token_merging import bipartite_merge, unmerge_tokens
        rng    = np.random.default_rng(1)
...
class TestTokenSwiftWiring:
    def test_import(self):
        from squish.token.token_swift import TokenSwiftConfig, TokenSwiftDecoder
        rng   = np.random.default_rng(0)
        vocab = 16
    ...
    def test_config_defaults(self):
        from squish.token.token_swift import TokenSwiftConfig
...
class TestC2TWiring:
    def test_import(self):
        from squish.token.c2t import AdaptiveTreeBuilder, C2TConfig
        cfg     = C2TConfig(tree_depth=3)
        builder = AdaptiveTreeBuilder(cfg)
        assert builder is not None

    def test_config_defaults(self):
        from squish.token.c2t import C2TConfig
        cfg = C2TConfig()
    ...
    def test_features_compute_shape(self):
        from squish.token.c2t import C2TFeatures
