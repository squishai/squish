...
# Qwen3-0.6B, squished for Apple Silicon

This is **Qwen3-0.6B** (0.6B parameters), compressed with
...
[mlx-community/Qwen3-0.6B-bf16](https://huggingface.co/mlx-community/Qwen3-0.6B-bf16).
...
brew install squish
squish run qwen3:0.6b
```

`squish run qwen3:0.6b` pulls these exact weights
(`squishai/Qwen3-0.6B-bf16-squished`) and starts a local OpenAI/Ollama-compatible server on
...
| Property | Value |
|----------|-------|
| Base model | [mlx-community/Qwen3-0.6B-bf16](https://huggingface.co/mlx-community/Qwen3-0.6B-bf16) |
| Developer | Alibaba Cloud |
| Parameters | 0.6B |
| Quantization | INT4 (4-bit, group size 64, affine) |
| Size on disk | **0.35 GB** squished, from 1.2 GB bf16 (~71% smaller) |
...
model, tokenizer = load("squishai/Qwen3-0.6B-bf16-squished")
